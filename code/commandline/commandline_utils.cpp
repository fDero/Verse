...
bool version_flag(int& i, int argc, char **argv, CommandLine& state){
    ...
    if (i + 1 < argc or !state.inputs.empty() or !state.outputs.empty() or state.mode != Mode::standard) 
        throw std::runtime_error("if you use the version flag (-v / --version) that's supposed to be the only one");
...
bool help_flag(int& i, int argc, char **argv, CommandLine& state){
    ...
    if (i + 1 < argc or !state.inputs.empty() or !state.outputs.empty() or state.mode != Mode::standard) 
        throw std::runtime_error("if you use the help flag (-h / --help) that's supposed to be the only one");
    state.mode = Mode::help;
    return true;
}

CommandLine read_commandline(int argc, char **argv){
    if (argc == 0 or argc == 1) throw std::runtime_error("don't know what to do"); 
    CommandLine state {{},{},Mode::standard};
    int i = 1;
    while (i < argc){
        ...
        throw std::runtime_error("unrecognized input: " + std::string(argv[i]));
    }
    return state;
}
